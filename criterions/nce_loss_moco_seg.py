...
class NCELossMocoSeg(nn.Module):
    """
    ...
    def __init__(self, config):
        super(NCELossMocoSeg, self).__init__()

        self.K = int(config["NUM_NEGATIVES"])
        self.dim = int(config["EMBEDDING_DIM"])
        self.T = float(config["TEMPERATURE"])

        self.register_buffer("queue_seg", torch.randn(self.dim, self.K)) # queue of dc (either pointnet or vox) based negative key embeddings
    ...
    def forward(self, output_dict):
        ...
        output_0 = output_dict[0]['seg_feats'] #query features (num_segments, 128)
        output_1 = output_dict[1]['seg_feats'] #key_features = moco features (num_segments, 128)

        normalized_output1 = nn.functional.normalize(output_0, dim=1, p=2) #query embedding
        normalized_output2 = nn.functional.normalize(output_1, dim=1, p=2) #key embedding
        ...
        # negative logits: NxK
        l_neg_s1q2 = torch.einsum('nc,ck->nk', [normalized_output1, self.queue_seg.clone().detach()])
