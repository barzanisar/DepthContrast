...
tmux 
scripts/submit_ddp_turing.sh --num_gpus 4 --cuda_visible_devices 0,1,2,3 --tcp_port 19832 --cfg_file configs/waymo_minkunet_depthcontrast_waymo10.yaml --pretrain_batchsize_per_gpu 32 --pretrain_epochs 50 --finetune_epochs 5 --finetune_batchsize_per_gpu 10 --model_name minkunet_pretrain_depthcontrast_waymo10 --pretrained_ckpt checkpoint-ep4.pth.tar > ./output/log/mink_pretrain_test_$(date +%Y-%m-%d_%H:%M).out 2>&1
