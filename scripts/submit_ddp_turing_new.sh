...
BASE_CMD_WAYMO="SINGULARITYENV_CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES
...
BASE_CMD_KITTI="SINGULARITYENV_CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES
SINGULARITYENV_WANDB_API_KEY=$WANDB_API_KEY
SINGULARITYENV_NCCL_BLOCKING_WAIT=1
singularity exec
--nv
...
BASE_CMD_NUSCENES="SINGULARITYENV_CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES
SINGULARITYENV_WANDB_API_KEY=$WANDB_API_KEY
SINGULARITYENV_NCCL_BLOCKING_WAIT=1
singularity exec
--nv
...
if [[ "$MODE" == "pretrain" ]]; then
    ...
    --downstream_model_dir fixed_scratch_"$FRAME_SAMPLING_DIV"percent_"$FINETUNE_EPOCHS"epochs"$TAG"
    --pretrained_ckpt checkpoint-ep0.pth.tar 
    --workers $WORKERS_PER_GPU 
    --frame_sampling_div $FRAME_SAMPLING_DIV
    "

    echo "Running Scratch training on Waymo"
    echo "$SCRATCH_CMD"
    eval $SCRATCH_CMD
    echo "Done scratch training on Waymo"



    SCRATCH_CFG_FILE=configs/semantickitti_scratch_$BACKBONE.yaml
    SCRATCH_CMD=$BASE_CMD_KITTI
    SCRATCH_CMD+="python -m torch.distributed.launch
    --nproc_per_node=$NUM_GPUS --nnodes=1 --node_rank=0 --master_addr=$MASTER_ADDR --master_port=$TCP_PORT --max_restarts=0
    /DepthContrast/tools/downstream_segmentation.py
    --launcher pytorch
...
if [[ "$MODE" == "finetune" ]]; then
    ...
    --epochs $FINETUNE_EPOCHS
    --batchsize_per_gpu $FINETUNE_BATCHSIZE_PER_GPU 
    --downstream_model_dir fixed_finetune_waymo_"$FRAME_SAMPLING_DIV"percent_"$FINETUNE_EPOCHS"epochs
    --model_name $MODEL_NAME
    --pretrained_ckpt $PRETRAINED_CKPT 
    --workers $WORKERS_PER_GPU 
    --frame_sampling_div $FRAME_SAMPLING_DIV
    "
    echo "Running Finetuning on Waymo"
    echo "$FINETUNE_CMD"
    eval $FINETUNE_CMD
    echo "Done Finetuning on Waymo"

    FINETUNE_CFG_FILE=configs/semantickitti_fine1lr_$BACKBONE.yaml
    FINETUNE_CMD=$BASE_CMD_KITTI
    ...
    --epochs $FINETUNE_EPOCHS
    --batchsize_per_gpu $FINETUNE_BATCHSIZE_PER_GPU 
    --downstream_model_dir fixed_finetune_semantickitti_"$FRAME_SAMPLING_DIV"percent_"$FINETUNE_EPOCHS"epochs
    --model_name $MODEL_NAME
    --pretrained_ckpt $PRETRAINED_CKPT 
    --workers $WORKERS_PER_GPU 
    --frame_sampling_div $FRAME_SAMPLING_DIV
    ...
    FINETUNE_CMD=$BASE_CMD_NUSCENES
    ...
    --epochs $FINETUNE_EPOCHS
    --batchsize_per_gpu $FINETUNE_BATCHSIZE_PER_GPU 
    --downstream_model_dir fixed_finetune_nuscenes_"$FRAME_SAMPLING_DIV"percent_"$FINETUNE_EPOCHS"epochs
    --model_name $MODEL_NAME
    --pretrained_ckpt $PRETRAINED_CKPT 
    --workers $WORKERS_PER_GPU 
    --frame_sampling_div $FRAME_SAMPLING_DIV
