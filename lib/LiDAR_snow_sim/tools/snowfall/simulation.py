...
import lib.LiDAR_snow_sim.tools.snowfall.geometry as g

from lib.LiDAR_snow_sim.tools.wet_ground.planes import calculate_plane
from lib.LiDAR_snow_sim.tools.wet_ground.augmentation import estimate_laser_parameters
    ...
    for j, beam_dict in enumerate(occlusion_list):
        ...
        if len(beam_dict.keys()) > 1:                           # otherwise there is no snowflake in the current beam
            ...
            if abs(d_max - d_orig) < 2 * (1 / intervals_per_meter):  # only alter intensity
                ...
                pc[j, 4] = 1 # attenuated object intensity bcz no snowflake's power exceeded object power
                ...
                pc[j, 4] = 2 # missing object point == replaced by snowflake
            ...
            pc[j, 4] = 0 #unchanged (neither attenuated nor replaced by snowflake) object point because no snowflake in beam
...
def get_occlusions(beam_angles: np.ndarray, ranges_orig: np.ndarray, root_path: str, particle_file: str,
    ...
    if root_path:
        path = Path(root_path) / 'snowflakes' / 'npy' / particle_file
    else:
