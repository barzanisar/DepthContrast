...
quantile_thresh = np.array([1, 5, 10, 20, 30, 40, 50])/100  #np.array([0.01, 0.03, 0.05, 0.1, 0.5, 1, 2, 3, 4, 5])/100 
...
def analyse_knns(dists_list, class_ids, sign):
    ...
    for i, thresh in enumerate(quantile_thresh):
        ...
        if len(dists_list) > 1:
            ...
            mask_selected_1 = dists_list[1] < iou_dist_thresh #row_wise_quantiles.repeat(1, n_samples)
...
for min_pts in shape_desc_min_pts:
    ...
    analyse_knns([esf_dist, iou_dist], class_ids_min_pts, f'esf_AND_iouthresh{iou_dist_thresh_name}-p{min_pts}-s{random_seed}')
...
signs = []
for min_pts in shape_desc_min_pts:
    signs += [f'esf_AND_iouthresh{iou_dist_thresh_name}-p{min_pts}-s{random_seed}']
    # signs += [f'iou-p{min_pts}-s{random_seed}', 
    #           f'esf-p{min_pts}-s{random_seed}',
    #           f'esf_AND_iouthresh{iou_dist_thresh_name}-p{min_pts}-s{random_seed}']
...
plt.grid()
plt.legend()
save_path = results_save_path / 'plots' / f'avg_prec_{sign}_knn.png'
