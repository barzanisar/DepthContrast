...
esf_dist_threshs = np.linspace(0.03, 0.05, 10)
...
def analyse_thresh(esf_dist, class_ids, sign):
...
for min_pts in shape_desc_min_pts:
    ...
    analyse_thresh(esf_dist, class_ids_min_pts, f'esf_thresh-p{min_pts}-s{random_seed}')
...
signs = []
for min_pts in shape_desc_min_pts:
    signs += [f'esf_thresh-p{min_pts}-s{random_seed}']
