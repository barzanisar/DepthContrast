...
def get_shape_desc(points, cluster_labels, info,  method):
    ...
    for i, lbl in enumerate(info['cluster_labels_boxes']):
        ...
        if len(obj_points) > 5: #TODO make this 20
    ...
    for desc_type in desc_types:
        ...
        for info_cluster in waymo_infos_cluster:
            ...
            if desc_type in ['iou', 'iou_z']:
                ...
                shape_desc_frame = get_shape_desc(points, pt_cluster_labels, info_cluster, method=desc_type) # n cluster in this frame, shape feat dim
...
def plotting(sign, class_ids):
    ...
    plt.savefig(f'classwise_avg_prec_{sign}_knn.png')
    plt.show()
...
# plt.figure()
# for sign in signs:
#     median_samplewise_acc_threshwise = np.zeros(len(quantile_thresh))
#     acc_samplewise_threshwise = pickle.load(open(f'acc_samplewise_threshwise_{sign}.pkl', 'rb'))
#     for i, thresh in enumerate(quantile_thresh):
#         median_samplewise_acc_threshwise[i] = np.median(acc_samplewise_threshwise[:, i])
#     plt.plot(quantile_thresh*100, median_samplewise_acc_threshwise, label=f'{sign}')


# plt.ylabel('Median Precision for nearest neighbors')
# plt.xlabel('Quantiles %')
# plt.title(f'Median Precision QNN')

# plt.grid()
# plt.legend()
# plt.savefig(f'median_prec_all_methods_knn.png')
# plt.show()